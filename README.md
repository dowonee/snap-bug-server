...
사용자의 React 애플리케이션에 삽입되어 컴포넌트 상태 변화와 DOM 구조 변경을 실시간으로 감지하는 스크립트입니다.<br>
React 내부 구조인 `__reactFiber$`와 `memoizedState`를 활용해 상태 변경을 감지하며 UI 복원을 위해 필요한 DOM/CSS 정보도 함께 기록합니다.

변경이 감지될 때마다 타임스탬프, 상태 값, DOM, CSS를 포함한 스냅샷을 생성하고 서버로 전송합니다.

## Web Viewer – 시점별 상태 및 UI 복원

기록된 상태 스냅샷을 기반으로 시간 순 상태 히스토리를 탐색하고 UI를 복원할 수 있는 디버깅 웹 뷰어입니다.<br>
사용자는 타임라인을 따라 상태 변화 흐름을 확인하고 각 시점에서 실제 UI가 어떻게 바뀌었는지 시각적으로 비교할 수 있습니다.

React와 Tailwind CSS 기반으로 구축되었으며 저장된 DOM과 CSS 정보로 실제 렌더링 결과를 재현할 수 있습니다.

## CLI Tool – 상태 추적 자동화 및 결과 공유
...
SDK로부터 전달된 상태 변화 데이터를 수신하고 시간, 상태, DOM, CSS 정보로 구성된 스냅샷을 저장합니다.<br>
